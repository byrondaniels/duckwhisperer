...
enum LocalStyleRewriter {
    ...
    static func robot(_ text: String) throws -> String {
        ...
        guard FileManager.default.fileExists(atPath: modelURL.path) else {
            throw LocalWhispererError.styleRewriteRuntimeMissing("Install the Enhanced Robot model from Speed & Accuracy.")
        }
    ...
    private static func run(runnerURL: URL, modelURL: URL, prompt: String) throws -> String {
        ...
        if waitGroup.wait(timeout: .now() + timeoutSeconds) == .timedOut {
            process.terminate()
            process.waitUntilExit()
            throw LocalWhispererError.styleRewriteFailed("The local robot model timed out.")
        }
        ...
        guard process.terminationStatus == 0 else {
            throw LocalWhispererError.styleRewriteFailed("llama-cli exited with \(process.terminationStatus).")
        }
